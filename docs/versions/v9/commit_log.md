...
## Uncommitted - v9-c12: add local monitoring dashboard HTML

### What Changed
- Added a static local monitoring dashboard renderer.
- Added a command to generate `reports/monitoring/dashboard.html`.
- Renders cards and sections from `reports/monitoring/dashboard_snapshot.json`.
- Added HTML escaping for snapshot-derived values.
- Updated README and V9 documentation.
- Added focused tests for dashboard rendering, persistence, validation, and docs.

### What Problem It Solved
- Turns the V9 dashboard data contract into an actual visible local dashboard.
- Gives the project dashboard visibility without installing Grafana, starting a server, or adding new dependencies.

### Verification
- `vir_env\Scripts\python.exe -m pytest -q tests\test_v9_c12_local_monitoring_dashboard_html.py` passed: `6 passed in 0.80s`.
- `vir_env\Scripts\python.exe -m py_compile app\observability\monitoring_dashboard.py app\build_monitoring_dashboard.py` passed.
- `vir_env\Scripts\python.exe -m app.build_monitoring_dashboard` generated `reports\monitoring\dashboard.html` with `9075` bytes.
...
## Uncommitted - v9-c15: add monitoring retention incident workflow

### What Changed
- Added a monitoring retention and incident debugging workflow.
- Documented how each V9 monitoring artifact is used during investigation.
- Documented the Git retention boundary for generated logs and reports.
- Documented the privacy boundary for telemetry retention.
- Added debugging paths for high failure rate, high latency, data drift, and empty Grafana dashboards.
- Updated README and V9 documentation.
- Added focused tests for the workflow document.

### What Problem It Solved
- Makes V9 observability useful after an incident by explaining how to reconstruct what happened from dashboards, metrics, reports, and raw telemetry.
- Clarifies what is retained locally, what is ignored by Git, and what remains outside V9.

### Verification
- `vir_env\Scripts\python.exe -m pytest -q tests\test_v9_c15_monitoring_retention_incident_workflow.py` passed: `5 passed in 0.04s`.
